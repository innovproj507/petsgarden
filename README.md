...
o menú, qué IA vas a usar (Claude o Gemini) y su API key, y con qué servicio vas a
conectar WhatsApp.
...
│   ├── brain.py             El cerebro — arma el contexto y llama a la IA
...
│   └── ai_providers/        Conexión con tu proveedor de IA
│       ├── base.py          Interfaz común
│       ├── __init__.py      Elige el proveedor automáticamente
│       └── anthropic.py     Adaptador (o gemini.py)
│
├── config/
...
**3. API key de tu proveedor de IA**
[platform.anthropic.com](https://platform.anthropic.com/settings/keys) para Claude
(recomendado, empieza con `sk-ant-...`) o [aistudio.google.com/apikey](https://aistudio.google.com/apikey)
para Gemini.
...
### Elegir tu proveedor de IA y modelo

El proveedor se elige con `AI_PROVIDER` (`anthropic` | `gemini`) y el modelo con
`ANTHROPIC_MODEL` o `GEMINI_MODEL` según cuál elijas — todo sin tocar código.

**Claude (Anthropic)** — recomendado, es con el que este sistema está más probado:
...
**Gemini (Google)** — alternativa, precios en [ai.google.dev/pricing](https://ai.google.dev/pricing):

| Modelo | ID | Cuándo usarlo |
|---|---|---|
| Gemini 3.5 Pro | `gemini-3.5-pro` | Razonamiento más profundo |
| **Gemini 3.5 Flash** | `gemini-3.5-flash` | **Default de Gemini.** Rápido y barato |
...
claude "Quiero probar Gemini en vez de Claude."
```
...
|---|---|---|
| IA | Claude o Gemini (`claude-sonnet-5` por default) | Genera las respuestas — tú eliges |
...
El sistema usa un **patrón adaptador** para los proveedores, tanto de WhatsApp como de IA:
cada uno implementa la misma interfaz, así que `main.py` no sabe ni le importa cuál de
WhatsApp estás usando (solo llama `proveedor.verificar_firma()`, `parsear_webhook()` y
`enviar_mensaje()`), y `brain.py` no sabe ni le importa si le habla a Claude o a Gemini
(solo llama `proveedor_ia.generar()`).
...
**¿Puedo cambiar de Claude a Gemini (o viceversa) después?**
Sí, es una variable en el `.env` (`AI_PROVIDER`). Las dos dependencias ya están instaladas
desde el principio para que el cambio no requiera reinstalar nada.
