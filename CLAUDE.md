...
## 0. Si estás editando el repo de AgentKit (no construyendo un agente)

Las secciones 1-8 de este archivo son el prompt operativo que Claude Code ejecuta cuando
alguien corre `/build-agent` para construir SU agente. Si en cambio te pidieron trabajar
sobre el repo `whatsapp-agentkit` en sí — arreglar `scripts/audit.py`, tocar `docs/index.html`,
ajustar `.claude/commands/build-agent.md` o este mismo archivo — ten en cuenta:

- **No hay `agent/`, `config/` ni `tests/` en este repo.** Esas carpetas las genera
  `/build-agent` dentro del proyecto del usuario final; acá solo existen como bloques de
  código Python/YAML dentro de la sección 3 de este archivo. El "código fuente" de AgentKit
  son esos bloques markdown, no un árbol de Python tradicional — por eso no hay
  requirements.txt, tests ni linter propios en la raíz del repo.
- **Validar cambios:** `python3 scripts/audit.py` (`--skip-links` para saltar el chequeo de
  red). Corre 6 chequeos: que los bloques de código Python de este archivo compilen, que
  los bloques YAML parseen, que no queden rastros de proveedores de WhatsApp ya
  descontinuados, que toda variable de entorno mencionada acá esté documentada en `.env.example`, y que los
  links e imágenes del README respondan. Córrelo antes de abrir un PR (ver "Contribuir" en
  el README).
- **Estructura del repo:**
  - `CLAUDE.md` — el cerebro; secciones 1-8 son las instrucciones que sigue Claude Code
  - `.claude/commands/build-agent.md` — resumen de las 5 fases, punto de entrada de `/build-agent`
  - `scripts/audit.py` — los 6 chequeos de integridad descritos arriba
  - `docs/` — sitio de marketing (`index.html`, `hero-card.html`)
  - `start.sh` — script que corre el usuario final antes de abrir Claude Code
  - `knowledge/` — carpeta vacía (solo `.gitkeep`); el usuario final sube ahí sus archivos
- **Si editas las secciones 1-8:** son instrucciones que Claude Code sigue literalmente
  durante el onboarding real de un usuario. Un cambio de texto ahí cambia el comportamiento
  del asistente para todos los que corran `/build-agent` después — revisa el impacto en las
  5 fases antes de tocar nada, y corre `scripts/audit.py` al terminar.
...
| IA | Anthropic Claude API / Google Gemini API | El usuario elige durante el setup (`AI_PROVIDER`) |
...
Tres dependencias que parecen opcionales y no lo son:
...
- **`asyncpg`**, aunque en local se use SQLite: en cuanto el usuario agrega PostgreSQL en
  ...
  agente no arranca.
- **`google-genai`**, aunque el usuario elija Claude como proveedor. Igual que con
  `asyncpg`: se instala siempre para que cambiar `AI_PROVIDER=anthropic` a
  `AI_PROVIDER=gemini` más adelante sea solo editar el `.env`, no reinstalar dependencias.

### 2.1 Proveedor y modelo de IA

El proveedor de IA se elige con `AI_PROVIDER` (`anthropic` | `gemini`, default `anthropic`).
Es el mismo patrón adaptador que los proveedores de WhatsApp (ver sección 3): `brain.py` no
sabe ni le importa cuál está activo, solo llama a la interfaz común de `agent/ai_providers/`.

**Si `AI_PROVIDER=anthropic`** — el modelo se elige con `ANTHROPIC_MODEL`, default `claude-sonnet-5`:
...
**Si `AI_PROVIDER=gemini`** — el modelo se elige con `GEMINI_MODEL`, default `gemini-3.5-flash`:

| Modelo | ID | Cuándo usarlo |
|---|---|---|
| Gemini 3.5 Pro | `gemini-3.5-pro` | Razonamiento más profundo, catálogos o reglas complejas |
| Gemini 3.5 Flash | `gemini-3.5-flash` | **Default.** Rápido y barato, sobra para atención a clientes |
| Gemini 3.5 Flash-Lite | `gemini-3.5-flash-lite` | El más barato, solo FAQ cortas |

Los precios de Gemini cambian seguido — confirma en https://ai.google.dev/pricing antes de
decirle un número al usuario en vez de repetir una cifra vieja.

**Los IDs de modelo de Gemini se descontinúan más rápido que los de Claude.** Google
retira generaciones enteras (la API responde `404 NOT_FOUND` con un mensaje del tipo
"ya no está disponible, usa modelos/gemini-X.Y-..."). Si al generar o probar el agente
la llamada a Gemini falla con 404, es casi seguro que el modelo quedó viejo: lee el ID
sugerido en el propio mensaje de error, actualiza el default en `GEMINI_MODEL` y en
`agent/ai_providers/gemini.py`, y avísale al usuario — no asumas que el número de esta
tabla sigue vigente sin haberlo probado.

Si el usuario no dice nada, usa `AI_PROVIDER=anthropic` con el modelo default. No cambies de
proveedor ni de modelo por tu cuenta para "ahorrar": es una decisión del dueño del negocio,
no tuya. Genera SOLO el adaptador de IA que el usuario eligió — no los dos.
...
│   ├── brain.py           ← Orquesta ai_providers/ + system prompt desde prompts.yaml
...
│   │   └── zernio.py      ← Adaptador del proveedor de WhatsApp elegido (o meta.py)
│   └── ai_providers/
│       ├── __init__.py    ← Factory: obtener_proveedor_ia() según .env
│       ├── base.py        ← Clase abstracta ProveedorIA
│       └── anthropic.py   ← Adaptador de IA elegido (o gemini.py)
├── config/
...
brain.py — arma system prompt + historial + mensaje nuevo
    ↓
ai_providers/ — se lo pasa al proveedor de IA elegido (Claude o Gemini)
    ↓
providers/ — envía la respuesta por el proveedor de WhatsApp elegido
    ↓
...
PREGUNTA 8: ¿Qué IA quieres que use tu agente?

            1. Claude / Anthropic (RECOMENDADO)
               Es el default de AgentKit: el que mejor probado está con este sistema.
            2. Gemini / Google

            Si el usuario no tiene preferencia o no sabe, sugiere Claude y sigue.
...
PREGUNTA 9: [Depende de la respuesta de PREGUNTA 8]
            ...
            Si eligió GEMINI:
                ...
                Si NO → Guiar paso a paso:
                         ...
                         4. Cópiala — no empieza con un prefijo fijo, es alfanumérica
...
**Regla clave:** genera SOLO los adaptadores que el usuario eligió — de WhatsApp y de IA.
Si eligió Zernio, no escribas `meta.py`; si eligió Claude, no escribas `gemini.py`, y
...
#### 3.7 — `agent/ai_providers/base.py` (siempre se genera)
...
#### 3.8 — `agent/ai_providers/__init__.py` (siempre se genera)
...
#### 3.9 — `agent/ai_providers/anthropic.py` (si eligió Claude)

```python
# agent/ai_providers/anthropic.py — Adaptador de IA: Anthropic Claude
# Generado por AgentKit

"""
Conexion con la API de Anthropic (Claude).
Documentacion: https://docs.claude.com
"""
...
class ProveedorAnthropic(ProveedorIA):
    ...
    async def generar(self, system_prompt: str, mensajes: list[dict]) -> tuple[str, dict]:
        """Genera una respuesta con Claude. Ver ProveedorIA.generar para el contrato."""
        extras = (
            {"output_config": {"effort": ESFUERZO}}
            if (self._soporta_esfuerzo and ESFUERZO)
            else {}
        )
        ...
        try:
            ...
            if extras and _es_error_de_esfuerzo(e):
                logger.warning(
                    f"El modelo {MODELO} no acepta output_config.effort; "
        ...
        return self._extraer_texto(respuesta), {
            "error": False,
            "cortado": getattr(respuesta, "stop_reason", None) == "max_tokens",
            "input_tokens": respuesta.usage.input_tokens,
            "output_tokens": respuesta.usage.output_tokens,
        }
```

#### 3.10 — `agent/ai_providers/gemini.py` (si eligió Gemini)
...
Genera SOLO las variables de los proveedores elegidos (IA y WhatsApp). Las del otro no
van, ni comentadas.
...
12. **Genera SOLO el adaptador del proveedor elegido** (de WhatsApp y de IA) — no los dos
13. **No cambies el modelo ni el proveedor de IA por tu cuenta** para ahorrar: es decisión del usuario
...
```env
# ── Proveedor de IA (anthropic | gemini, default anthropic) ─
AI_PROVIDER=anthropic
...
GEMINI_API_KEY=
GEMINI_MODEL=gemini-3.5-flash       # gemini-3.5-pro | gemini-3.5-flash | gemini-3.5-flash-lite
# GEMINI_MAX_TOKENS=4096            # opcional, default 4096
